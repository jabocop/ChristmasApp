...
var LoginCtrl = (function (_super) {
    ...
    LoginCtrl.prototype.submit = function () {
        var _this = this;
        this.$http.post('/authenticate', this.$scope.user).success(function (data, status, headers, config) {
            _this.$window.sessionStorage.setItem("token", data.token);
            _this.$scope.isAuthenticated = true;
            var encodedProfile = data.token.split('.')[1];
            var urlEnoder = new urlDecoder();
            var profile = JSON.parse(urlEnoder.url_base64_decode(encodedProfile));
            _this.$scope.welcome = 'Welcome ' + profile.first_name + ' ' + profile.last_name;
        }).error(function (data, status, headers, config) {
            // Erase the token if the user fails to log in
            delete _this.$window.sessionStorage.removeItem("token");

            // Handle login errors here
            _this.$scope.message = 'Error: Invalid user or password';
        });
    };
